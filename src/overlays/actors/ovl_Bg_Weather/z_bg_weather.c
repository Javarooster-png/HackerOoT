 ...
 * Tuning knobs
 */
#define RAIN_FOG_ENABLED 1
#define NIGHT_MIST_ENABLED 1
...
#define NIGHT_MIST_FOLLOW_PLAYER_Y 1
...
static const u8 sRainFogLayerOrder[RAIN_FOG_NUM_LAYERS] = {
    1,
    RAIN_FOG_GROUND_LAYER,
};

static const FogLayer sNightMistLayers[NIGHT_MIST_LAYERS] = {
    { NIGHT_MIST_LAYER0_HEIGHT, NIGHT_MIST_LAYER0_SPREAD, NIGHT_MIST_LAYER0_WIDTH,
      NIGHT_MIST_LAYER0_HEIGHT_SIZE, NIGHT_MIST_LAYER0_ALPHA, (u64*)gDust5Tex, 2, 1 },
    { NIGHT_MIST_LAYER1_HEIGHT, NIGHT_MIST_LAYER1_SPREAD, NIGHT_MIST_LAYER1_WIDTH,
      NIGHT_MIST_LAYER1_HEIGHT_SIZE, NIGHT_MIST_LAYER1_ALPHA, (u64*)gDust3Tex, -3, 2 },
};

void BgWorld_Init(Actor* thisx, PlayState* play);
void BgWorld_Destroy(Actor* thisx, PlayState* play);
void BgWorld_Update(Actor* thisx, PlayState* play);
void BgWorld_Draw(Actor* thisx, PlayState* play);
...
static f32 BgWorld_AbsF(f32 value) {
    return (value < 0.0f) ? -value : value;
}

static f32 BgWorld_MaxF(f32 a, f32 b) {
    return (a > b) ? a : b;
}

static f32 BgWorld_Lerpf(f32 a, f32 b, f32 t) {
    return a + (b - a) * t;
}

static s32 BgWorld_FloorToGridCell(f32 value, f32 cellSize) {
...
static f32 BgWorld_HashToUnit(s32 layer, s32 gx, s32 gz, s32 salt) {
...
static void BgWorld_GetWindStep(PlayState* play, f32 fallbackX, f32 fallbackZ, f32* stepX, f32* stepZ) {
...
static f32 BgWorld_GetPuffFade(f32 rFrac, f32 uFrac) {
    f32 edge = 1.0f - BgWorld_MaxF(BgWorld_AbsF(rFrac), BgWorld_AbsF(uFrac));
    f32 radial = 1.0f - CLAMP((SQ(rFrac) + SQ(uFrac)) * 0.5f, 0.0f, 1.0f);

    return edge * edge * (0.35f + 0.65f * radial);
}

static f32 BgWorld_GetPuffGlowFade(f32 rFrac, f32 uFrac) {
    f32 fade = BgWorld_GetPuffFade(rFrac, uFrac);

    return CLAMP((fade * 0.72f) + (sqrtf(fade) * 0.28f), 0.0f, 1.0f);
}

static f32 BgWorld_GetPlayerY(PlayState* play, f32 fallbackY) {
    Actor* player = play->actorCtx.actorLists[ACTORCAT_PLAYER].head;

    return (player != NULL) ? player->world.pos.y : fallbackY;
}

static f32 BgWorld_GetAnchorY(BgWorld* this, PlayState* play) {
    return this->actor.world.pos.y + (BgWorld_GetPlayerY(play, this->basePlayerY) - this->basePlayerY);
}

static f32 BgWorld_GetNightFactor(u16 time) {
    if (time >= CLOCK_TIME(18, 30)) {
        return CLAMP((time - CLOCK_TIME(18, 30)) / (f32)(CLOCK_TIME(19, 30) - CLOCK_TIME(18, 30)), 0.0f, 1.0f);
    }

    if (time < CLOCK_TIME(4, 30)) {
        return 1.0f;
    }

    if (time < CLOCK_TIME(5, 30)) {
        return 1.0f - CLAMP((time - CLOCK_TIME(4, 30)) / (f32)(CLOCK_TIME(5, 30) - CLOCK_TIME(4, 30)), 0.0f, 1.0f);
    }

    return 0.0f;
}

static void BgWorld_UpdateLocalDayTime(BgWorld* this, PlayState* play) {
    if (play->envCtx.sceneTimeSpeed != 0) {
        this->localDayTime = gSaveContext.save.dayTime;
    } else {
        this->localDayTime += 0x10;
    }
}

void BgWorld_Init(Actor* thisx, PlayState* play) {
    BgWorld* this = (BgWorld*)thisx;

    this->rainFogIntensity = RAIN_FOG_INTENSITY;
    this->localDayTime = gSaveContext.save.dayTime;
    this->basePlayerY = BgWorld_GetPlayerY(play, thisx->world.pos.y);
    this->nightMistIntensity = BgWorld_GetNightFactor(this->localDayTime);
...
void BgWorld_Update(Actor* thisx, PlayState* play) {
    ...
    BgWorld_UpdateLocalDayTime(this, play);
    Math_SmoothStepToF(&this->nightMistIntensity, BgWorld_GetNightFactor(this->localDayTime), 0.08f, 0.02f, 0.001f);
    Math_SmoothStepToF(&this->rainFogIntensity, RAIN_FOG_INTENSITY, 0.12f, 0.03f, 0.001f);
}

static void BgWorld_LoadFogState(GraphicsContext* gfxCtx, Gfx** gfxP, u8 primR, u8 primG, u8 primB) {
...
static void BgWorld_DrawFogPuffGrid(GraphicsContext* gfxCtx, Gfx** gfxP, Vtx* vtx, s32 drawCount, s32 vertsPerPuff,
                                    s32 puffSubdiv, u64* tex) {
    Gfx* gfx = *gfxP;
    s32 pi;
    ...
    gDPPipeSync(gfx++);
    gDPLoadTextureBlock(gfx++, tex, G_IM_FMT_I, G_IM_SIZ_8b, 32, 32, 0, G_TX_CLAMP, G_TX_CLAMP, 5, 5, G_TX_NOLOD,
                        G_TX_NOLOD);
...
#if NIGHT_MIST_ENABLED
static void BgWorld_DrawNightMist(BgWorld* this, PlayState* play) {
...
#if NIGHT_MIST_FOLLOW_PLAYER_Y
    groundY = BgWorld_GetAnchorY(this, play);
#else
    groundY = this->actor.world.pos.y;
#endif
    BgWorld_GetWindStep(play, NIGHT_MIST_WIND_SPEED_X, NIGHT_MIST_WIND_SPEED_Z, &windStepX, &windStepZ);

    {
        f32 fwdX = play->mainCamera.at.x - eyeX;
        f32 fwdZ = play->mainCamera.at.z - eyeZ;
        f32 fwdLen = sqrtf(fwdX * fwdX + fwdZ * fwdZ);
    ...
    OPEN_DISPS(gfxCtx, "../z_bg_weather.c", __LINE__);

    BgWorld_LoadFogState(gfxCtx, &gfx, NIGHT_MIST_PRIM_R, NIGHT_MIST_PRIM_G, NIGHT_MIST_PRIM_B);

    Matrix_Push();
    Matrix_Translate(eyeX, eyeY, eyeZ, MTXMODE_NEW);
    MATRIX_FINALIZE_AND_LOAD(gfx++, gfxCtx, "../z_bg_weather.c", __LINE__);
    ...
    for (li = 0; li < NIGHT_MIST_LAYERS; li++) {
        ...
        s32 baseCellX = BgWorld_FloorToGridCell(eyeX - windX, spacing) - (NIGHT_MIST_GRID_DIM / 2);
        s32 baseCellZ = BgWorld_FloorToGridCell(eyeZ - windZ, spacing) - (NIGHT_MIST_GRID_DIM / 2);
        u8 baseAlpha = (u8)CLAMP(layer->peakAlpha * mistIntensity * NIGHT_MIST_ALPHA_SCALE, 0.0f,
                                 NIGHT_MIST_MAX_ALPHA);
        Vtx* vtx;
        s32 drawCount = 0;
        s32 gx;
        s32 gz;
        ...
        for (gx = 0; gx < NIGHT_MIST_GRID_DIM; gx++) {
            for (gz = 0; gz < NIGHT_MIST_GRID_DIM; gz++) {
                ...
                f32 jitterX = (BgWorld_HashToUnit(9 + li, cellX, cellZ, 0x1729) - 0.5f) * spacing * 0.34f;
                f32 jitterZ = (BgWorld_HashToUnit(9 + li, cellX, cellZ, 0x283D) - 0.5f) * spacing * 0.34f;
                f32 scale = 0.85f + BgWorld_HashToUnit(9 + li, cellX, cellZ, 0x39AF) * 0.35f;
                f32 dist;
                f32 distFrac;
                f32 nearFrac;
                f32 rx;
                ...
                localX += jitterX;
                localZ += jitterZ;
                localX += Math_SinS((s16)(play->gameplayFrames * (75 + li * 13) +
                                           BgWorld_HashToUnit(9 + li, cellX, cellZ, 0x51ED) * 65535.0f)) *
                          NIGHT_MIST_WIND_SWAY;
                localZ += Math_CosS((s16)(play->gameplayFrames * (61 + li * 11) +
                                           BgWorld_HashToUnit(9 + li, cellX, cellZ, 0x6B4F) * 65535.0f)) *
                          NIGHT_MIST_WIND_SWAY;
                ...
                puffHalfH = layer->puffHalfH * (0.85f + BgWorld_HashToUnit(9 + li, cellX, cellZ, 0x4CE1) * 0.25f);
                ...
                for (ri = 0; ri < NIGHT_MIST_PUFF_SUBDIV; ri++) {
                    ...
                    for (ui = 0; ui < NIGHT_MIST_PUFF_SUBDIV; ui++) {
                        ...
                        v->v.cn[3] = (u8)(puffAlpha * BgWorld_GetPuffGlowFade(rFrac, uFrac));
                    }
                }
                drawCount++;
            }
        ...
        BgWorld_DrawFogPuffGrid(gfxCtx, &gfx, vtx, drawCount, NIGHT_MIST_VERTS_PER_PUFF, NIGHT_MIST_PUFF_SUBDIV,
                                layer->tex);
    }

    Matrix_Pop();
    POLY_XLU_DISP = gfx;
    CLOSE_DISPS(gfxCtx, "../z_bg_weather.c", __LINE__);
}
#endif

#if RAIN_FOG_ENABLED
static void BgWorld_DrawRainFog(BgWorld* this, PlayState* play) {
    GraphicsContext* gfxCtx = play->state.gfxCtx;
    Gfx* gfx;
    f32 eyeX = play->mainCamera.eye.x;
    f32 eyeY = play->mainCamera.eye.y;
    f32 eyeZ = play->mainCamera.eye.z;
    f32 groundY = BgWorld_GetAnchorY(this, play);
    f32 heightFactor = 1.0f - ((eyeY - groundY) / 1200.0f);
    f32 dayFogFactor = 1.0f - BgWorld_GetNightFactor(this->localDayTime);
    ...
    BgWorld_GetWindStep(play, RAIN_FOG_WIND_SPEED_X, RAIN_FOG_WIND_SPEED_Z, &windStepX, &windStepZ);
    ...
    fr = (u8)CLAMP(BgWorld_Lerpf((f32)play->envCtx.lightSettings.fogColor[0], 232.0f, RAIN_FOG_PRIM_BLEND), 180.0f,
                   245.0f);
    fg = (u8)CLAMP(BgWorld_Lerpf((f32)play->envCtx.lightSettings.fogColor[1], 236.0f, RAIN_FOG_PRIM_BLEND), 190.0f,
                   245.0f);
    fb = (u8)CLAMP(BgWorld_Lerpf((f32)play->envCtx.lightSettings.fogColor[2], 242.0f, RAIN_FOG_PRIM_BLEND), 200.0f,
                   250.0f);
    ...
    BgWorld_LoadFogState(gfxCtx, &gfx, fr, fg, fb);
    ...
    for (li = 0; li < RAIN_FOG_NUM_LAYERS; li++) {
        ...
        s32 baseCellX = BgWorld_FloorToGridCell(eyeX - windX, cellSize) - (FOG_GRID_DIM / 2);
        s32 baseCellZ = BgWorld_FloorToGridCell(eyeZ - windZ, cellSize) - (FOG_GRID_DIM / 2);
        u8 baseAlpha = (u8)CLAMP(layer->peakAlpha * this->rainFogIntensity * heightFactor * dayFogAlphaBoost * opacity,
                                 0.0f, RAIN_FOG_MAX_ALPHA);
        Vtx* vtx;
        s32 drawCount = 0;
        s32 gx;
        s32 gz;
        ...
        for (gx = 0; gx < FOG_GRID_DIM; gx++) {
            for (gz = 0; gz < FOG_GRID_DIM; gz++) {
                ...
                f32 jitterX = (BgWorld_HashToUnit(layerIndex, cellX, cellZ, 0x0471) - 0.5f) * cellSize * 0.62f;
                f32 jitterZ = (BgWorld_HashToUnit(layerIndex, cellX, cellZ, 0x0937) - 0.5f) * cellSize * 0.62f;
                f32 scale = 1.02f + BgWorld_HashToUnit(layerIndex, cellX, cellZ, 0x1231) * 0.38f;
                f32 alphaScale = 0.90f + BgWorld_HashToUnit(layerIndex, cellX, cellZ, 0x1B57) * 0.45f;
                f32 localX;
                f32 localZ;
                f32 dist;
                f32 distFrac;
                ...
                worldCenterX += Math_SinS((s16)(play->gameplayFrames * (68 + layerIndex * 17) +
                                                BgWorld_HashToUnit(layerIndex, cellX, cellZ, 0x51ED) * 65535.0f)) *
                                RAIN_FOG_WIND_SWAY;
                worldCenterZ += Math_CosS((s16)(play->gameplayFrames * (57 + layerIndex * 19) +
                                                BgWorld_HashToUnit(layerIndex, cellX, cellZ, 0x6B4F) * 65535.0f)) *
                                RAIN_FOG_WIND_SWAY;
                ...
                rx = billRightX * layer->puffHalfW * scale;
                rz = billRightZ * layer->puffHalfW * scale;
                puffHalfH = layer->puffHalfH * (0.88f + BgWorld_HashToUnit(layerIndex, gx, gz, 0x2C13) * 0.28f);
                ...
                for (ri = 0; ri < FOG_PUFF_SUBDIV; ri++) {
                    ...
                    for (ui = 0; ui < FOG_PUFF_SUBDIV; ui++) {
                        ...
                        v->v.ob[0] = (s16)(localX + rFrac * rx);
                        v->v.ob[1] = (s16)(deltaY + uFrac * puffHalfH +
                                           (BgWorld_HashToUnit(layerIndex, cellX, cellZ, 0x45CD) - 0.5f) * 95.0f);
                        v->v.ob[2] = (s16)(localZ + rFrac * rz);
                        v->v.flag = 0;
                        v->v.tc[0] = (s16)((rFrac + 1.0f) * 512.0f);
                        v->v.tc[1] = (s16)((uFrac + 1.0f) * 512.0f);
        ...
        BgWorld_DrawFogPuffGrid(gfxCtx, &gfx, vtx, drawCount, FOG_VERTS_PER_PUFF, FOG_PUFF_SUBDIV, layer->tex);
    }
...
#if RAIN_FOG_ENABLED
    BgWorld_DrawRainFog(this, play);
#endif
#if NIGHT_MIST_ENABLED
    BgWorld_DrawNightMist(this, play);
#endif
}
